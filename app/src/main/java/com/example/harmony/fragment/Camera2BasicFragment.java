...
import com.example.harmony.view.AutoFitTextureView;
...
//    };
    ...
    private void openCamera(int width, int height) {
        ...
        setUpCameraOutputs(width, height); // CameraCharacteristics.LENS_FACING_FRONT 초기값은 전면
        configureTransform(width, height);
        Activity activity = getActivity();
        CameraManager manager = (CameraManager) activity.getSystemService(Context.CAMERA_SERVICE);
        try {
        ...
        try {
            ...
            CameraCaptureSession.CaptureCallback CaptureCallback
                ...
                public void onCaptureCompleted(@NonNull CameraCaptureSession session,
                    ...
                    //showToast("Saved: " + mFile); //사진을 찍었을때 토스트메세지
            ...
            Task<Uri> urlTask = uploadTask.continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
                ...
                public void onComplete(@NonNull Task<Uri> task) {
                    if (task.isSuccessful()) {
                        ...
                        Log.e("파일 업로드 성공", "파일 업로드 성공 :" + downloadUri);
                    } else {
                        // Handle failures
                        Log.e("파일 업로드 실패2", "파일 업로드 실패2");
                    }
                }
            });
